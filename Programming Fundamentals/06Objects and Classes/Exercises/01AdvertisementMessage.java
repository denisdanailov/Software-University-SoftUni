...
public class AdvertisementMessage {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < messages; i++) {
            ...
            int randPhrase = rand.nextInt(phrase.length);
            int randEvent = rand.nextInt(events.length);
            int randAuthor = rand.nextInt(authors.length);
            int randCity = rand.nextInt(cities.length);

            System.out.println(phrase[randPhrase] + " " + events[randEvent] + " " + authors[randAuthor] + " - " + cities[randCity]);
        }
